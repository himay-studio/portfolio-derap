#!/usr/bin/env node
/**
 * Harness QA terukur untuk Derap.
 *
...
const KATA_KAPITAL_SAH = [
  ...
  // Istilah teknis yang memang ditulis begitu di data demo Derap.
  'DevOps', 'IndexedDB', 'QRIS', 'PageSpeed', 'localStorage',
];
...
async function utama() {
  ...
  try {
    ...
    // Rute unik per pola, supaya sapuan penuh tidak menghabiskan waktu di 47
    // halaman detail yang markup-nya identik.
    const pola = new Map();
    for (const r of dipakai) {
      const kunci = r.replace(/\/(t\d+|[a-z0-9-]+)\/$/, (m, seg) =>
        /^(app|tugas|proyek|sprint|tim|timesheet|pengaturan|status|label|login)$/.test(seg) ? m : '/:x/',
      );
